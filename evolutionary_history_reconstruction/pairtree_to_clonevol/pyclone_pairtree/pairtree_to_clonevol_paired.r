...
#paired sample stayed paired and unpaired samples stayed unpaired
...
meta$subject_id<-gsub("_.*$","",meta$sampleid)
...
meta$sample_rt<-paste(meta$sampleid ,meta$sequenceofsamplevRT, sep = "_")
#meta$sample_id<-paste(meta$unique_sample_id , meta$sequenceofsamplevRT , sep = "_")
#meta$subject_id<-paste(meta$unique_sample_id , meta$sequenceofsamplevRT , sep = "_")


########### make a tree.tsv file #######
### start from pairtree XX.tree.json file
file_trees<-list.files("~/Dropbox/cancer_reserach/sarcoma/sarcoma_analysis/pairtree_to_clonevol/trees_final_March08/batch1_paired_unpaired/jsons", pattern = "*.tree.json", full.names = TRUE)
...
for (ii in 1:length(file_trees)){
     ...
     for(kk in 1:length(json_focal$phi)){
      ...
      write.table(out_res_cell, file = paste("~/Dropbox/cancer_reserach/sarcoma/sarcoma_analysis/pairtree_to_clonevol/trees_final_March08/batch1_paired_unpaired/outputs/tree/tree.",subject,".tsv", sep = ""), col.names = TRUE, row.names = FALSE, sep = "\t", quote = FALSE)
...
py_data_good<-merge(py_data_good,meta,by.x ="sample_id" , by.y = "sampleid")[c("Chrom" , "Pos" ,"Ref" ,"Alt" ,"sample_id" ,"cluster_id" ,"cellular_prevalence", "cellular_prevalence_std", "cluster_assignment_prob", "subject_id.x","pos_id" ,"sample_rt")]
colnames(py_data_good)[colnames(py_data_good)=="subject_id.x"]<-"subject_id"
...
variants_aa<-merge(variants_aa,meta,by.x ="sample_id" , by.y = "sampleid")[c("sample_id", "Chromosome" ,"Start" , "End" ,"Ref" ,"Alt","Coding" , "Gene", "subject_id.x" ,"gene", "is.driver" , "pos_id" ,"sample_rt")]
colnames(variants_aa)[colnames(variants_aa)=="subject_id.x"]<-"subject_id"
...
out_res_var<-NULL
for(ss in 1:length(subject_id)){     ### loop through each subject
  #out_res_all<-NULL
    ...
    names(py_focal_tokeep_renamed)[6]<-"cluster_id"   ##### py_focal_tokeep_renamed is a pyclone file format ###
    ...
    for(rr in 1:length(cluster_0)){   
     #for(rr in 1:5){    ### add fake cluster 0 muttaions and rbind with pyclone  (keep only 5 cluetr 0 muttaions)
        ...
        sudo_positions_focal<-sudo_positions_focal[,c("Chromosome","Start","Ref","Alt","sample_id","subject_id.x","pos_id", "cluster_id", "cellular_prevalence", "cellular_prevalence_std","cluster_assignment_prob","sample_rt")]
        ...
        sudo_positions_focal<-sudo_positions_focal[,c("Chromosome", "Start", "Ref", "Alt", "sample_id","cluster_id" ,"cellular_prevalence" ,"cellular_prevalence_std", "cluster_assignment_prob"  ,  "subject_id.x" ,"pos_id","a")]

        colnames(sudo_positions_focal)<-c("Chrom" , "Pos" ,"Ref", "Alt", "sample_id" ,"cluster_id", "cellular_prevalence" ,"cellular_prevalence_std" ,"cluster_assignment_prob" ,"subject_id" ,"pos_id" ,"sample_rt")
...
write.table(out_res_var, file = "~/Dropbox/cancer_reserach/sarcoma/sarcoma_analysis/pairtree_to_clonevol/trees_final_March08/batch1_paired_unpaired/outputs/out_res_var_pyclone_format_with_cluster0.txt", col.names = TRUE, row.names = FALSE, sep = "\t", quote = FALSE)
...
for (tt in 1:length(file_trees)){
     ...
     json_focal <- fromJSON(paste(readLines(focal_file_tree), collapse=""))  ## parse Json tree files into R

     ## adjust samples name to match with variant file
     subject<-gsub(".tree.json", "",sub(".*/", "", focal_file_tree)) 
     sample_id<-json_focal$samples
     samples<-paste(subject,sample_id, sep = "_")
     samples_right<-data.frame("sample"=gsub("R_","",samples))
...
colnames(out_res_all_phi)[colnames(out_res_all_phi)=="sample"]<-"sample_rt"
out_res_all_phi$subject_id<-gsub("_.*$","",out_res_all_phi$sample_rt)
out_res_all_phi$sample_id<-sub("_preRT|_postRT|_noRT","",out_res_all_phi$sample)
out_res_all_phi$clust_sample_id<-paste(out_res_all_phi$sample_rt , out_res_all_phi$cluster_id , sep = "_")
...
write.table(out_res_all_phi, file = "~/Dropbox/cancer_reserach/sarcoma/sarcoma_analysis/pairtree_to_clonevol/trees_final_March08/batch1_paired_unpaired/outputs/out_res_all_paired_phi.txt", col.names = TRUE, row.names = FALSE, sep = "\t", quote = FALSE)
...
for (zz in 1:length(subject_id)){
    ...
    #mutaion_RT_focal_good_col<-mutaion_RT_focal[,c("code","cluster_id","cellular_prevalence","pos_identifier","sample_id","clust_sample_id")]
     ...
     mutaion_RT_focal_good<-merge(py_focal,out_res_all_phi_focal, by.x = "clust_sample_id", by.y = "clust_sample_id")[,c("subject_id.x","cluster_id.x","pairtree_cff","sample_rt.x","pos_id")]
    ...
    focal_pos<-mutaion_RT_focal_good[mutaion_RT_focal_good$pos_id== pos_identifier[ii],]
     ...
     if(length(unique(focal_pos$sample_rt.x))< length(unique(out_res_all_phi_focal$sample_rt))){
         ...
         focal_pos$sample_rt.x[duplicated(focal_pos$sample_rt.x)] <- find_not_included
    ...
    #write.table(file = "out_res_phy", file = paste("~/Desktop/out_res_variants_clonevol_format_",subject_id[zz],".txt"), col.names = TRUE, row.names = FALSE, sep = "\t", quote = FALSE)
     ...
     write.table(out_res_phy, file = paste("~/Dropbox/cancer_reserach/sarcoma/sarcoma_analysis/pairtree_to_clonevol/trees_final_March08/batch1_paired_unpaired/outputs/variants_fixed_missing/variants.",subject_id[zz],".tsv", sep = ""), col.names = TRUE, row.names = FALSE, sep = "\t", quote = FALSE)
...
file_trees<-list.files("~/Dropbox/cancer_reserach/sarcoma/sarcoma_analysis/pairtree_to_clonevol/trees_final_March08/batch1_paired_unpaired/outputs/tree", pattern = "*.tsv", full.names = TRUE)
file_vars<-list.files("~/Dropbox/cancer_reserach/sarcoma/sarcoma_analysis/pairtree_to_clonevol/trees_final_March08/batch1_paired_unpaired/outputs/variants_fixed_missing", pattern = "*.tsv", full.names = TRUE)
...
for (ii in 1:length(file_trees)){
      ...
      tree_title<-gsub("/Users/shsoudi/Dropbox/cancer_reserach/sarcoma/sarcoma_analysis/pairtree_to_clonevol/trees_final_March08/batch1_paired_unpaired/outputs/tree/", "",gsub(".tsv","",file_trees[ii]))


      pdf(file = paste("~/Dropbox/cancer_reserach/sarcoma/sarcoma_analysis/pairtree_to_clonevol/trees_final_March08/batch1_paired_unpaired/outputs//clonevol_tree_",tree_title,".pdf", sep = ""), width = 7, height = 7)
