...
file_trees<-list.files("~/Dropbox/cancer_reserach/sarcoma/sarcoma_analysis/pairtree_to_clonevol/April_updated1/pre_post/jsons", pattern = "*.tree.json", full.names = TRUE)
...
for (ii in 1:length(file_trees)){
     ...
     for(kk in 1:length(json_focal$phi)){
      ...
      write.table(out_res_cell, file = paste("~/Dropbox/cancer_reserach/sarcoma/sarcoma_analysis/pairtree_to_clonevol/April_updated1/pre_post/outputs/trees/tree.",subject,".tsv", sep = ""), col.names = TRUE, row.names = FALSE, sep = "\t", quote = FALSE)
...
### load pyc.json files 
file_pyc<-list.files("~/Dropbox/cancer_reserach/sarcoma/sarcoma_analysis/pairtree_to_clonevol/April_updated1/pre_post/pyc.jsons", pattern = "*pyc.out.json", full.names = TRUE)
...
############################################################
############################################################
filenames_py <- list.files("~/Dropbox/cancer_reserach/sarcoma/sarcoma_analysis/pairtree_to_clonevol/April_updated1/pre_post/ssms",pattern="*.ssm", full.names = TRUE)  ### load pyclone cluster files
...
for (ss in 1:length(subject)){
     ...
     focal_samples<-unique(meta_focal$sample_rt)
...
#  id subject_id      pos_id Chrom       Pos Ref Alt       sample_rt cluster_id
#1 s0     TB9051 1_101339681  chr1 101339681   G   T TB9051_9_postRT          6
#2 s0     TB9051 1_101339681  chr1 101339681   G   T TB9051_13_preRT          6
#3 s0     TB9051 1_101339681  chr1 101339681   G   T TB9051_5_postRT          6
#4 s0     TB9051 1_101339681  chr1 101339681   G   T TB9051_12_preRT          6
...
write.table(out_res_all_phi, file = "~/Dropbox/cancer_reserach/sarcoma/sarcoma_analysis/pairtree_to_clonevol/April_updated1/pre_post/outputs/out_res_all_paired_phi.txt", col.names = TRUE, row.names = FALSE, sep = "\t", quote = FALSE)
...
write.table(out_res_var, file = "~/Dropbox/cancer_reserach/sarcoma/sarcoma_analysis/pairtree_to_clonevol/April_updated1/pre_post/outputs/out_res_var_paired_seperated_pairtree_pyclone_format_with_cluster0.txt", col.names = TRUE, row.names = FALSE, sep = "\t", quote = FALSE)
...
for (zz in 1:length(subject_id)){
    ...
    #mutaion_RT_focal_good_col<-mutaion_RT_focal[,c("code","cluster_id","cellular_prevalence","pos_identifier","sample_id","clust_sample_id")]
     ...
     names(mutaion_RT_focal_good)[colnames(mutaion_RT_focal_good)=="sample_rt.x"]<-"sample_rt"
   ...
   variant_focal_good<-variant_focal[!duplicated(variant_focal$gene),]   ### added
    ...
    #write.table(file = "out_res_phy", file = paste("~/Desktop/out_res_variants_clonevol_format_",subject_id[zz],".txt"), col.names = TRUE, row.names = FALSE, sep = "\t", quote = FALSE)
     ...
     write.table(out_res_phy, file = paste("~/Dropbox/cancer_reserach/sarcoma/sarcoma_analysis/pairtree_to_clonevol/April_updated1/pre_post/outputs/variants/variants.",subject_id[zz],".tsv", sep = ""), col.names = TRUE, row.names = FALSE, sep = "\t", quote = FALSE)
...
file_trees<-list.files("~/Dropbox/cancer_reserach/sarcoma/sarcoma_analysis/pairtree_to_clonevol/April_updated1/pre_post/outputs/trees", pattern = "*.tsv", full.names = TRUE)
file_vars<-list.files("~/Dropbox/cancer_reserach/sarcoma/sarcoma_analysis/pairtree_to_clonevol/April_updated1/pre_post/outputs/variants", pattern = "*.tsv", full.names = TRUE)
...
for (ii in 1:length(file_trees)){
      ...
      tree_title<-gsub("/Users/shsoudi/Dropbox/cancer_reserach/sarcoma/sarcoma_analysis/pairtree_to_clonevol/April_updated1/pre_post/outputs/trees/", "",gsub(".tsv","",file_trees[ii]))


      pdf(file = paste("/Users/shsoudi/Dropbox/cancer_reserach/sarcoma/sarcoma_analysis/pairtree_to_clonevol/April_updated1/pre_post/outputs/clonevol_trees/clonevol_tree_",tree_title,".pdf", sep = ""), width = 7, height = 7)
